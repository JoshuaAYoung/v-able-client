...
export default class RegistrationPage extends Component {
  ...
  generateError = () => {
    ...
    const errorProps = {};
    if (
      this.state.emailTempValue.touched === true &&
      !emailRegEx.test(emailTempValue.value)
    ) {
      errorProps.emailError = 'Please enter a valid email address.';
    }
    if (
      this.state.passwordTempValue.touched === true &&
      !passRegEx.test(passwordTempValue.value)
    ) {
      errorProps.passwordError1 =
        'Password must contain an upper case letter, a lower case letter and a number.';
    }
    if (
      this.state.passwordTempValue.touched === true &&
      passwordTempValue.value.length < 8
    ) {
      errorProps.passwordError2 =
        'Password must be equal to or longer than 8 characters.';
    }
    if (
      this.state.passwordTempValue.touched === true &&
      passwordTempValue.value.length > 71
    ) {
      errorProps.passwordError3 = 'Password must be less than 72 characters.';
    }
    if (
      this.state.passwordTempValue.touched === true &&
      (passwordTempValue.value.startsWith(' ') ||
        passwordTempValue.value.endsWith(' '))
    ) {
      errorProps.passwordError4 =
        'Password must not start or end with empty spaces.';
    }
    if (
      this.state.zipcodeTempValue.touched === true &&
      (zipcodeTempValue.value.length !== 5 ||
        !zipcodeRegEx.test(zipcodeTempValue.value))
    ) {
